...
@include('layouts.komponen.backend.src_blueimp')
    ...
    @if(file_exists(public_path('upload/avatars/'.md5($user->email).'.jpg')))
        <img src="/upload/avatars/{!! md5($user->email) !!}.jpg?{!! date('YmdHis') !!}"   id='gambar' class='img-rounded'   style='width:100px;height:100px;border:3px solid #aaa;' />
    @else
...
<div class="fileUpload btn btn-primary">
    ...
    <input data-form-data='{"_token": "{!! csrf_token() !!}", "email" : "{!! $user->email !!}" }' 
        id="fileupload" 
        type="file"
        name="files" 
        data-url="{!! URL::route('users.do_change_avatar') !!}" 
...
//function start here
   ...
   var skrg_int = skrg.getDate()+""+skrg.getHours()+""+skrg.getMinutes()+""+skrg.getSeconds();
    ...
    $('#fileupload').val('');
       ...
       $('#gambar').attr('src', '/upload/avatars/{!! md5($user->email).".jpg?" !!}'+skrg_int);
